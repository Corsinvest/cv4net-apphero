...
public static class ServiceCollectionExtensions
{
    ...
    private static IServiceCollection ConfigureHangfire(this IServiceCollection services)
    {
        ...
        var path = Path.Combine(ApplicationHelper.PathData, "hangfire.db");

        var useEfCore = false;
        if (useEfCore)
        {
            GlobalConfiguration.Configuration
                               .UseEFCoreStorage(builder => builder.UseSqlite(DataBaseHelper.CreateSQLitePath(path)), new EFCoreStorageOptions())
                               .UseDatabaseCreator();

        }
        else
        {
            GlobalConfiguration.Configuration.UseStorage(new SQLiteStorage(path));
        }
        return services;
    }
